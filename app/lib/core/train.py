...
def train_model():
    ...
    try:
        ...
        if hasattr(model, 'attention_layer'):
            try:
                print("\n=== ATTENTION ANALYSIS ===")
                feature_importance = model.analyze_attention_patterns(features)
                attention_analysis = {
        ...
        return {
            ...
            "model_architecture": "wide_deep_with_attention" if hasattr(model, 'attention_layer') else "wide_deep_basic",
            ...
            "note": "Model trained with engagement score (continuous) and attention mechanism successfully"
        }
